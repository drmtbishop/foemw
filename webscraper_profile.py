...
#from requests_futures import sessions - didn't work
#requests_session = sessions.FuturesSession()

# Single requests session to reduce time in 'calls'
...
def wa():
	...
	wa_data = BeautifulSoup(wa_htmlcode, 'lxml')
...
def jw():
	...
	jw_data = BeautifulSoup(jw_htmlcode, 'lxml')
...
def gw():
	...
	for pageNumber in range(30):
		...
		gw_data = BeautifulSoup(gw_htmlcode, 'lxml')
...
def swa():	
	...
	swa_data = BeautifulSoup(swa_htmlcode, 'lxml')
...
#	try:
#		swa_lastpage = int(swa_pagelist[-2].contents[0])
#	except IndexError:
#		swa_lastpage = 1
#
	#print ('Getting Scotch Whisky Auctions data from '+str(swa_lastpage)+' total page(s)...')
	...
	for eachpage in range(swa_lastpage):
		...
		for entry in swa_auctionlist:
			...
			#print (entry.find('a').find('h4').text)
			try:
				tempdict['price']=float(entry.find('p', {'class':'sold'}).get_text().split(u"\xA3",1)[1].replace("," , ""))
			except AttributeError:
				break
			#print (float(entry.find('p', {'class':'sold'}).get_text().split(u"\xA3",1)[1].replace("," , "")))

			tempdict['lot']=entry.find('h6').text.split(' ',2)[2]
			#print (entry.find('a').find('h6').text.split(' ',2)[2])
	...
	#return results_plot(swadata, 'Scotch Whisky Auctions')
	#print ("SWA Records: "+str(len(swadata)))
	#print (swadata)
	return swadata
...
if __name__ == '__main__':
	...
	#cProfile.run('multiplot(wa(), wh(), jw(), gw(), swa())' , 'app.profile')
